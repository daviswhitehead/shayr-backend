...
const backfillUsersPosts = async (
  db: any,
  batcher: Batcher,
  userAId: documentId,
  userBId: documentId,
  ...
  if (actionResults && !_.isEmpty(actionResults.documents)) {
    _.forEach(actionResults.documents, (value, key) => {
      batcher.set(
        db.collection('users_posts').doc(`${userBId}_${value.postId}`),
        {
          [`${action}`]: arrayUnion(firebase.firestore, userAId),
          createdAt: value.createdAt || ts(firebase.firestore),
          postId: value.postId || '',
...
export const onUpdateFriendship = async (
  ...
  // backfilling for the initiating user
  await backfillUsersPosts(
    db,
    batcher,
    receivingUser._id,
